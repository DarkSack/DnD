...
import useAuth from "@/hooks/useAuth"; // Asegúrate de que este hook exista y provea info de sesión
...
const AuthComponent = () => {
  ...
  const handleLogin = async () => {
    if (user) Alert({ title: "Error", text: "Ya estas logueado", icon: "error" });
    ...
    try {
      ...
      Alert({ title: "Exito", text: "Inicio de sesión exitoso", icon: "success" });
    } catch (error) {
  ...
  const handleRegister = async () => {
    ...
    try {
      await new Promise((resolve) => setTimeout(resolve, 1500));
      Alert({ title: "Exito", text: "Registro exitoso", icon: "success" });
    } catch (error) {
      Alert({ title: "Error", text: error as string, icon: "error" });
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {/* Toggle entre Login y Registro con animaciones */}
